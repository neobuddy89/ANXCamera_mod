...
public class LastAnimationComponent {
    private HashMap<View, Float> mAlphaMap = new HashMap<>(8);
    ...
    public void hideOtherViews(int i, List<ImageView> list) {
        if (!this.mShown) {
            ...
            HashMap<View, Float> hashMap = this.mAlphaMap;
            if (hashMap == null) {
                this.mAlphaMap = new HashMap<>(8);
            } else {
                hashMap.clear();
            }
            for (View next : list) {
                ...
                if ((topConfigItem == null || topConfigItem.configItem != i) && next.getVisibility() == 0 && next.getAlpha() != 0.0f) {
                    ...
                    this.mAlphaMap.put(next, Float.valueOf(next.getAlpha()));
    ...
    public boolean reverse(boolean z) {
        ...
        if (list != null) {
            for (View next : list) {
                if (z) {
                    float f2 = 1.0f;
                    if (this.mAlphaMap.containsKey(next)) {
                        f2 = this.mAlphaMap.get(next).floatValue();
                    }
                    ViewCompat.setAlpha(next, 0.0f);
                    ViewCompat.animate(next).setStartDelay(0).setDuration(280).alpha(f2).translationX(0.0f).setInterpolator(this.mQuartEaseOut).start();
