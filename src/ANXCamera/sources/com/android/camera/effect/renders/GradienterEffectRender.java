...
public class GradienterEffectRender extends PixelEffectRender {
    ...
    private void filteRotation(float f2) {
        ...
        if (f3 != -1.0f) {
            if (Math.abs(f2 - f3) > 180.0f) {
                ...
                if (f2 < f4) {
                    ...
                    this.mLastRotation = f4 + 360.0f;
                }
            }
            this.mLastRotation = (this.mLastRotation * 0.7f) + (0.3f * f2);
        } else {
    ...
    public void initShaderValue(boolean z) {
        ...
        float f2 = ((float) this.mPreviewWidth) * 0.5f;
        float f3 = ((float) this.mPreviewHeight) * 0.5f;
        if (ModuleManager.isSquareModule()) {
            ...
            f3 -= (float) ((this.mSquareModeExtraMargin * this.mPreviewWidth) / Util.sWindowWidth);
        } else {
