...
class FastScroller extends RecyclerView.ItemDecoration implements RecyclerView.OnItemTouchListener {
    ...
    private void horizontalScrollTo(float f2) {
        ...
        float max = Math.max((float) horizontalRange[0], Math.min((float) horizontalRange[1], f2));
    ...
    private void verticalScrollTo(float f2) {
        ...
        float max = Math.max((float) verticalRange[0], Math.min((float) verticalRange[1], f2));
    ...
    public void updateScrollPosition(int i, int i2) {
        ...
        if (this.mNeedVerticalScrollbar || this.mNeedHorizontalScrollbar) {
            if (this.mNeedVerticalScrollbar) {
                float f2 = (float) i3;
                this.mVerticalThumbCenterY = (int) ((f2 * (((float) i2) + (f2 / 2.0f))) / ((float) computeVerticalScrollRange));
                this.mVerticalThumbHeight = Math.min(i3, (i3 * i3) / computeVerticalScrollRange);
            }
            if (this.mNeedHorizontalScrollbar) {
                float f3 = (float) i4;
                this.mHorizontalThumbCenterX = (int) ((f3 * (((float) i) + (f3 / 2.0f))) / ((float) computeHorizontalScrollRange));
