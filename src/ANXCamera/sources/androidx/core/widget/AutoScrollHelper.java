...
public abstract class AutoScrollHelper implements View.OnTouchListener {
    ...
    private static class ClampedScroller {
        ...
        private float getValueAt(long j) {
            ...
            return (1.0f - f2) + (f2 * AutoScrollHelper.constrain(((float) (j - j2)) / ((float) this.mEffectiveRampDown), 0.0f, 1.0f));
        }
    ...
    private float constrainEdgeValue(float f2, float f3) {
        ...
        if (i == 0 || i == 1) {
            if (f2 < f3) {
                return f2 >= 0.0f ? 1.0f - (f2 / f3) : (!this.mAnimating || this.mEdgeType != 1) ? 0.0f : 1.0f;
            }
        } else if (i == 2 && f2 < 0.0f) {
            return f2 / (-f3);
        }
